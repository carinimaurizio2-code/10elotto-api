...
def calcola_statistiche(tutte_estrazioni):
    ...
    top_rit  = sorted(ritardi.items(), key=lambda x: -x[1])[:10]
    top_oro  = sorted(freq_oro.items(), key=lambda x: -x[1])[:5]
    ...
    return {
        ...
        "top_frequenti": [{"numero": k, "frequenza": v} for k, v in top_freq],
        "top_ritardi":   [{"numero": k, "ritardo": v} for k, v in top_rit],
        "top_oro":       [{"numero": k, "frequenza": v} for k, v in top_oro],
